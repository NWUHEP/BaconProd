...
//--------------------------------------------------------------------------------------------------
FillerJet_REDUCED::FillerJet_REDUCED(const edm::ParameterSet &iConfig, const bool useAOD,edm::ConsumesCollector && iC):
  fMinPt              (iConfig.getUntrackedParameter<double>("minPt",20)),
  fMaxEta             (iConfig.getUntrackedParameter<double>("maxEta",3)),
...
//--------------------------------------------------------------------------------------------------
FillerJet_REDUCED::~FillerJet_REDUCED()
{
  delete fJetCorr;
  delete fJetUnc;
...
void FillerJet_REDUCED::initPUJetId() { 
  if(!fUseAOD) return;
...
void FillerJet_REDUCED::initBReg(){
  fBReg.initialize("BDT",fBRegFile);
}
void FillerJet_REDUCED::initBRegNN(){
  fBRegNN.initialize(fBRegNNFile,fBRegNNMean,fBRegNNStd);
}
void FillerJet_REDUCED::initBoostedBtaggingJetId(){
  fJetBoostedBtaggingMVACalc.initialize("BDT",fWeightFile,fWeightFile.find("Subjet") < std::string::npos);

}
//--------------------------------------------------------------------------------------------------
void FillerJet_REDUCED::initJetCorr(const std::vector<std::string> &jecFiles,
...
Measurement1D FillerJet_REDUCED::vertexDxy(const reco::VertexCompositePtrCandidate &svcand, const reco::Vertex &pv)  {
...
Measurement1D FillerJet_REDUCED::vertexD3d(const reco::VertexCompositePtrCandidate &svcand, const reco::Vertex &pv)  {
...
//--------------------------------------------------------------------------------------------------
// === filler for AOD ===
void FillerJet_REDUCED::fill(TClonesArray *array, TClonesArray *iExtraArray,TClonesArray *iSVArray,
                     ...
                     const pat::TriggerObjectStandAloneCollection *patTriggerObjects)
{
  ...
  for(reco::PFJetCollection::const_iterator itJet = jetCol->begin(); itJet!=jetCol->end(); ++itJet) {
    ...
    if(fabs(itJet->eta()) > fMaxEta) continue;
...
//--------------------------------------------------------------------------------------------------
void FillerJet_REDUCED::addJet(baconhep::TAddJet *pAddJet, TClonesArray *iSVArr,const reco::Vertex &pv, const edm::Event &iEvent, 
...
void FillerJet_REDUCED::addJet(baconhep::TAddJet *pAddJet, TClonesArray *iSVArr, const reco::Vertex &pv, const edm::Event &iEvent, const pat::Jet &itJet) {
  //pAddJet->pullAngle = JetTools::jetPullAngle(itJet,hSubJetProduct,fConeSize);
...
//--------------------------------------------------------------------------------------------------
const reco::PFJet* FillerJet_REDUCED::matchPF( const reco::PFJet *iJet,const reco::PFJetCollection *jets ) { 
  int lId = -1;
  double dRmin = 999.;
...
const reco::BasicJet* FillerJet_REDUCED::match( const reco::PFJet *iJet,const reco::BasicJetCollection *jets ) { 
  int lId = -1;
  double dRmin = 999.;
  for ( unsigned int i=0; i< jets->size(); ++i ) {
...
const reco::BasicJet* FillerJet_REDUCED::match(const pat::Jet *iJet, const reco::BasicJetCollection *jets) {
  int lId = -1;
  double dRmin = 999.;
  for ( unsigned int i=0; i< jets->size(); ++i ) {
    const reco::BasicJet* jet = &(*jets)[i];
...
const reco::GenJet* FillerJet_REDUCED::match( const reco::PFJet *iJet,const reco::GenJetCollection *jets ) { 
  int lId = -1;
  for ( unsigned int i=0; i< jets->size(); ++i ) {
...
const reco::GenJet* FillerJet_REDUCED::match(const pat::Jet *iJet, const reco::GenJetCollection *jets) {
  int lId = -1;
  for ( unsigned int i=0; i< jets->size(); ++i ) {
    const reco::GenJet *jet = &(*jets)[i];
    float dR = deltaR( iJet->eta(), iJet->phi(), jet->eta(), jet->phi() );
...
void FillerJet_REDUCED::softdrop(const reco::GenJet *iJet,float &iMsd,float &ie2,float &ie3) {
  std::vector<fastjet::PseudoJet>  lClusterParticles;
